...
function renderPager(controlId, infoId, current, pages, total, perPage, onClick) {
    ...
    if (pages <= 7) {
        ...
        for (let i = Math.max(2, current - 1); i <= Math.min(pages - 1, current + 1); i++) buttons.push(i);
    ...
    buttons.forEach(label => {
        if (label === '...') {
            ...
            span.textContent = '...';
            span.style.cssText = 'padding: 0 8px; color: #999;';
            container.appendChild(span);
        } else {
            const btn = document.createElement('button');
            btn.textContent = label;
            btn.className = `page-btn ${label === current ? ' active' : ''}`;
            btn.onclick = () => onClick(label);
